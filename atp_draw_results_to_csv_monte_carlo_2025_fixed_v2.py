...
ROUND_HEADING_PATTERNS = {
    "R128": [r"(?<![A-Za-z-])Round of 128(?![A-Za-z-])"],
    "R64": [r"(?<![A-Za-z-])Round of 64(?![A-Za-z-])"],
    "R32": [r"(?<![A-Za-z-])Round of 32(?![A-Za-z-])"],
    "R16": [r"(?<![A-Za-z-])Round of 16(?![A-Za-z-])"],
    "QF": [r"(?<![A-Za-z-])Quarter-Finals(?![A-Za-z-])", r"(?<![A-Za-z-])Quarterfinals(?![A-Za-z-])"],
    "SF": [r"(?<![A-Za-z-])Semi-Finals(?![A-Za-z-])", r"(?<![A-Za-z-])Semifinals(?![A-Za-z-])"],
    "F": [r"(?<![A-Za-z-])Finals(?![A-Za-z-])", r"(?<![A-Za-z-])Final(?![A-Za-z-])"],
}
...
def find_round_heading_position(draw_html: str, round_code: str, start_at: int = 0) -> int:
    positions = []

    for pattern in ROUND_HEADING_PATTERNS[round_code]:
        match = re.search(pattern, draw_html[start_at:], flags=re.I)
        if match:
            positions.append(start_at + match.start())
...
def detect_first_round_code(draw_html: str) -> str:
    for code in ("R128", "R64", "R32"):
        if find_round_heading_position(draw_html, code) != -1:
            return code
...
def slice_draw_html_for_round(draw_html: str, round_code: str) -> str:
    start = find_round_heading_position(draw_html, round_code)
    if start == -1:
    ...
    for next_code in ROUND_CODES_IN_ORDER[start_idx + 1:]:
        next_pos = find_round_heading_position(draw_html, next_code, start + 1)
        if next_pos != -1:
...
if __name__ == "__main__":
    DRAW_URL = "https://www.atptour.com/en/scores/archive/monte-carlo/410/2025/draws"

    BASE_DIR = Path(__file__).resolve().parent
    OUTPUT_CSV = BASE_DIR / "monte_carlo_2025_draw.csv"
