...
def parse_score_tokens(lines: Iterable[str]) -> list[int]:
    """
    Extract one score per set line.

    ATP pages sometimes expose tie-break details on the same visual score line,
    e.g. ``7(3)`` or ``6 3`` after text extraction. We only care about the
    first game-count number for each set, and must ignore tie-break points.
    """
    tokens: list[int] = []
    for line in lines:
...
def sets_won_from_score_raw(score_raw: str) -> tuple[int, int] | None:
    ...
    # Ignore tie-break points completely: 7-6(3) -> 7-6
    cleaned = re.sub(r"\([^)]*\)", "", score_raw)
...
def sets_for_row_order(row: MatchRow, result: MatchResult) -> tuple[int, int] | None:
    ...
    # Prefer the per-player score columns when present. On ATP pages these are
    # often more reliable than the free-text sentence and already ignore the
    # tie-break sub-score visually shown on the same line.
